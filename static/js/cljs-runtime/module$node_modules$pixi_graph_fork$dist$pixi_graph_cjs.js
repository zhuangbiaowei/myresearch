...
nodeStyle.border.width;var _a=["NODE_LABEL_TEXT",nodeStyle.label.fontFamily,nodeStyle.label.fontSize,nodeStyle.label.content].join("::");textureCache=textureCache.get(_a,function(){var type=nodeStyle.label.type,content=nodeStyle.label.content,JSCompiler_object_inline_fontFamily_4589=nodeStyle.label.fontFamily,JSCompiler_object_inline_fontSize_4590=nodeStyle.label.fontSize;if(type===exports.TextType.TEXT)type=new text.Text(content,{fontFamily:JSCompiler_object_inline_fontFamily_4589,fontSize:JSCompiler_object_inline_fontSize_4590,
fill:16777215,align:"center",lineJoin:"round",wordWrap:!0,wordWrapWidth:240});else if(type===exports.TextType.BITMAP_TEXT)type=new textBitmap.BitmapText(content,{fontName:JSCompiler_object_inline_fontFamily_4589,fontSize:JSCompiler_object_inline_fontSize_4590});else throw Error("Invalid state");type.roundPixels=!0;return type});var nodeLabelBackground=nodeLabelGfx.getChildByName("NODE_LABEL_BACKGROUND");nodeLabelBackground.y=_b+(textureCache.height+2*nodeStyle.label.padding)/2;nodeLabelBackground.width=
...
PixiGraph.prototype.updateNodeStyle=function(nodeKey,nodeAttributes){nodeKey=this.nodeKeyToNodeObject.get(nodeKey);nodeKey.updatePosition({x:nodeAttributes.x,y:nodeAttributes.y});nodeAttributes=resolveStyleDefinitions([JSCompiler_object_inline_node_4591,this.style.node,nodeKey.hovered?this.hoverStyle.node:void 0],nodeAttributes);nodeKey.updateStyle(nodeAttributes,this.textureCache)};PixiGraph.prototype.resetNodeStyle=function(nodeKey,nodeStyle){this.nodeKeyToNodeObject.get(nodeKey).updateNodeStyle(nodeStyle,
...
this.updateEdgeStyle(edgeKey,edgeAttributes,sourceNodeKey,targetNodeKey,sourceNodeAttributes,targetNodeAttributes)};PixiGraph.prototype.updateEdgeStyle=function(edgeKey,edgeAttributes,_sourceNodeKey,_targetNodeKey,sourceNodeAttributes,targetNodeAttributes){edgeKey=this.edgeKeyToEdgeObject.get(edgeKey);edgeKey.updatePosition({x:sourceNodeAttributes.x,y:sourceNodeAttributes.y},{x:targetNodeAttributes.x,y:targetNodeAttributes.y});edgeAttributes=resolveStyleDefinitions([JSCompiler_object_inline_edge_4592,
