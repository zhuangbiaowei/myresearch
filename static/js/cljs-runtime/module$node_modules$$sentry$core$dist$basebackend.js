...
function(_exception,_hint){throw new utils_1.SentryError("Backend has to implement `eventFromException` method");};BaseBackend.prototype.eventFromMessage=function(_message,_level,_hint){throw new utils_1.SentryError("Backend has to implement `eventFromMessage` method");};BaseBackend.prototype.sendEvent=function(event){void this._transport.sendEvent(event).then(null,function(reason){utils_1.isDebugBuild()&&utils_1.logger.error("Error while sending event:",reason)})};BaseBackend.prototype.sendSession=
function(session){this._transport.sendSession?void this._transport.sendSession(session).then(null,function(reason){utils_1.isDebugBuild()&&utils_1.logger.error("Error while sending session:",reason)}):utils_1.isDebugBuild()&&utils_1.logger.warn("Dropping session because custom transport doesn't implement sendSession")};BaseBackend.prototype.getTransport=function(){return this._transport};BaseBackend.prototype._setupTransport=function(){return new noop_1.NoopTransport};return BaseBackend}();exports.BaseBackend=
global}
