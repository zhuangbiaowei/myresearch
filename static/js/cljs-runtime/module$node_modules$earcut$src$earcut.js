...
holeIndices[hasHoles]*dim$jscomp$0;var end=hasHoles<len-1?holeIndices[hasHoles+1]*dim$jscomp$0:data.length;start=linkedList(data,start,end,dim$jscomp$0,!1);start===start.next&&(start.steiner=!0);end=outerNode;var JSCompiler_temp_const=end.push,p=start,leftmost=start;do{if(p.x<leftmost.x||p.x===leftmost.x&&p.y<leftmost.y)leftmost=p;p=p.next}while(p!==start);JSCompiler_temp_const.call(end,leftmost)}outerNode.sort(compareX);for(hasHoles=0;hasHoles<outerNode.length;hasHoles++)dim$jscomp$0=outerNode[hasHoles],
holeIndices=y,(y=findHoleBridge(dim$jscomp$0,holeIndices))?(dim$jscomp$0=splitPolygon(y,dim$jscomp$0),len=filterPoints(y,y.next),filterPoints(dim$jscomp$0,dim$jscomp$0.next),y=holeIndices===y?len:holeIndices):y=holeIndices,y=filterPoints(y,y.next);outerNode=y}if(data.length>80*dim){var minX=maxX=data[0];var minY=hasHoles=data[1];for(dim$jscomp$0=dim;dim$jscomp$0<outerLen;dim$jscomp$0+=dim)holeIndices=data[dim$jscomp$0],y=data[dim$jscomp$0+1],holeIndices<minX&&(minX=holeIndices),y<minY&&(minY=y),holeIndices>
maxX&&(maxX=holeIndices),y>hasHoles&&(hasHoles=y);maxX=Math.max(maxX-minX,hasHoles-minY);maxX=0!==maxX?1/maxX:0}earcutLinked(outerNode,triangles,dim,minX,minY,maxX);return triangles}function linkedList(data,start,end,dim,clockwise){if(clockwise===0<signedArea(data,start,end,dim))for(clockwise=start;clockwise<end;clockwise+=dim)var last=insertNode(clockwise,data[clockwise],data[clockwise+1],last);else for(clockwise=end-dim;clockwise>=start;clockwise-=dim)last=insertNode(clockwise,data[clockwise],data[clockwise+
1],last);last&&equals(last,last.next)&&(removeNode(last),last=last.next);return last}function filterPoints(start,end){if(!start)return start;end||=start;do{var again=!1;if(start.steiner||!equals(start,start.next)&&0!==area(start.prev,start,start.next))start=start.next;else{removeNode(start);start=end=start.prev;if(start===start.next)break;again=!0}}while(again||start!==end);return end}function earcutLinked(ear,triangles,dim,minX,minY,invSize,pass){if(ear){if(!pass&&invSize){var start=ear,p=start;
