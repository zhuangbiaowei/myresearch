...
this.name="\x3cunlabeled transaction\x3e"),_super.prototype.finish.call(this,endTimestamp),!0!==this.sampled)utils_1.logger.log("[Tracing] Discarding transaction because its trace was not chosen to be sampled."),(endTimestamp=(endTimestamp=this._hub.getClient())&&endTimestamp.getTransport&&endTimestamp.getTransport())&&endTimestamp.recordLostEvent&&endTimestamp.recordLostEvent("sample_rate","transaction");else return endTimestamp=this.spanRecorder?this.spanRecorder.spans.filter(function(s){return s!==
_this&&s.endTimestamp}):[],this._trimEnd&&0<endTimestamp.length&&(this.endTimestamp=endTimestamp.reduce(function(prev,current){return prev.endTimestamp&&current.endTimestamp?prev.endTimestamp>current.endTimestamp?prev:current:prev}).endTimestamp),endTimestamp={contexts:{trace:this.getTraceContext()},spans:endTimestamp,start_timestamp:this.startTimestamp,tags:this.tags,timestamp:this.endTimestamp,transaction:this.name,type:"transaction",sdkProcessingMetadata:this.metadata},0<Object.keys(this._measurements).length&&
